...
{
    public sealed class KaspaJob<TResponse> : Job
    {
...
----------------------------------------------------------------- */
        ...
        public KaspaJob(Func<CancellationToken, Task<Either<ErrorRecord, TResponse>>> process_task, string command_name) : base(command_name)
        {
            lock (this._lock)
            {
