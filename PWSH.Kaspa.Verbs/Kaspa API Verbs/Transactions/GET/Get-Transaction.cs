...
namespace PWSH.Kaspa.Verbs
{
    ...
    /// </summary>
    [Cmdlet(KaspaVerbNames.Get, "Transaction")]
    [OutputType(typeof(ResponseSchema))]
    public sealed partial class GetTransaction : KaspaPSCmdlet
    {
...
----------------------------------------------------------------- */
        ...
        protected override string BuildQuery()
        {
            ...
            queryParams["inputs"] = IncludeInputs.IsPresent.ToString().ToLower();
            queryParams["outputs"] = IncludeOutputs.IsPresent.ToString().ToLower();
            queryParams["resolve_previous_outpoints"] = ResolvePreviousOutpoints.ToString().ToLower();
            if (!string.IsNullOrEmpty(BlockHash)) queryParams["blockHash"] = BlockHash.ToString();

            return$"transactions/{TransactionID}?" + queryParams.ToString();
        }
